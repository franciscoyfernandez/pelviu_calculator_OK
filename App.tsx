...
const App: React.FC = () => {
  const [step, setStep] = useState<'welcome' | 'quiz' | 'lead-capture' | 'calculating' | 'results' | 'how-it-works' | 'admin'>('welcome');
  const [results, setResults] = useState<AssessmentResult | null>(null);
  const [genderView, setGenderView] = useState<'mujer' | 'hombre' | null>(null);
  const [tempQuizData, setTempQuizData] = useState<{ answers: Record<number, number>; gender: Gender } | null>(null);
  const [currentAssessmentId, setCurrentAssessmentId] = useState<string | null>(null);
  const [currentContact, setCurrentContact] = useState<LeadData | null>(null);
  ...
  const handleLeadCapture = (contactData: LeadData) => {
    if (!tempQuizData) return;
    ...
    setTimeout(() => {
      ...
      const newId = storage.createAssessment(gender, answers, calculated, contactData);
  ...
  return (
    <div className="min-h-screen flex flex-col selection:bg-[#EE8866]/30">
      {step !== 'admin' && <Header onOpenHowItWorks={openHowItWorks} onGoHome={restart} />}

      <main className="flex-grow pt-16">
        {step === 'admin' ? (
          <AdminDashboard onExit={restart} />
        ) : (
        ...
        <footer className="py-12 border-t border-gray-100 bg-white">
          <div className="max-w-5xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <button onClick={() => setStep('admin')} className="text-xs text-gray-300 hover:text-[#EE8866] underline">
                Acceso Profesional
              </button>
            </div>
